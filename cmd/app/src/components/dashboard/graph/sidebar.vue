<template>
	<v-navigation-drawer width="858" disable-resize-watcher
		mini-variant-width="60" :mini-variant="!open"
		id="sidebar-graph" class="panel graph-panel">
			...
			<v-list-item class="header" two-line>
				...
				<v-list-item-action>
					<v-row align="center" class="px-2">
						<v-btn icon small>
							<v-icon small color="grey lighten-1">fa-solid fa-circle-nodes</v-icon>
						</v-btn>
		...
		<template v-else>
			...
			<v-divider></v-divider>
			<v-icon small class="text-center pa-5" color="grey darken-2">fa-solid fa-circle-nodes</v-icon>
			<h4 class="vertical mono text-uppercase caption py-4 grey--text">Open Graph</h4>
		</template>
	</v-navigation-drawer>
</template>
...
export default {
	name: 'sidebar-graph',
	components: {
		graph: require('@/components/dashboard/graph/graph').default
	},
	computed: {
